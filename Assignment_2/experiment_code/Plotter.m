...
output.initE = -1/2*p.e1^2*p.Kp - 1/2*p.e2^2*p.Kp
output.totalE = [];
output.controllerE = 0;
tArray = [];
while (toc<5)
    ...
    output.controllerE = output.controllerE + -z1(2)*T1*dt + -z1(4)*T2*dt;
    tArray = [tArray; tnew];
    output.totalE = [output.totalE, output.controllerE];
end
output.controllerE
figure(2);
plot(tArray, output.totalE);
% title("PID Control");
title("PD Control");
xlabel("t(sec)");
ylabel("Total Energy");
